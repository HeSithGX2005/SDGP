...
.sidebar {
    ...
    padding-top: 20px;
    padding-left: 20px;
    border-top-right-radius: 20px; /* Add top-right rounded corner */
    border-bottom-right-radius: 20px; /* Add bottom-right rounded corner */
}
...
@media (max-width: 768px) {
    .sidebar {
        ...
        overflow-x: hidden;
        overflow-y: hidden;
        transition: width 0.10s ease; /* Add a smooth transition effect */
    }
    ...
    .dropdown-item {
    color: white; /* Set color for dropdown items */
}
...
<body>
<button class="btn btn-primary d-md-none toggle-btn">
                            ...
                            switch($user_role){
                                ...
                                case 'company':
                                    $dynamicLinks = array(
                                        ...
                                        array("url" => "message.php", "icon" => "fa-message", "text" => "Message"),
                                        array("text" => "Settings", "icon" => "fa-gear", "submenu" => array(
                                            array("url" => "changepassword.php", "icon" => "fa-lock","text" => "Password"),
                                            array("url" => "report.php",  "icon" => "fa-bell", "text" => "Report")
                                        ))
                                        // Add more dynamic links here
                                    );
                                    break;
                                case 'admin':
                                    $dynamicLinks = array(
                                ...
                                $iconPaths = array(
                                    ...
                                    "fa-lock" =>  "<path d = 'M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z'></path>"
                                );  
...
function sidebarShowing($dynamicLinks, $iconPaths) {
    foreach ($dynamicLinks as $link) {
        if (isset($link['submenu']) && is_array($link['submenu'])) {
            ...
            foreach ($link['submenu'] as $submenu_item) {
                echo "<li><a href='{$submenu_item['url']}' class='dropdown-item'>";
                // Output submenu item icon if available
                if (isset($submenu_item['icon']) && isset($iconPaths[$submenu_item['icon']])) {
                    echo "<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='fa-solid {$submenu_item['icon']}' viewBox='0 0 640 512'>
                            {$iconPaths[$submenu_item['icon']]}
                          </svg>";
                }
                echo "{$submenu_item['text']}</a></li>";
            }
